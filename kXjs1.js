...
async function downloadImage(event, imageUrl) {
    ...
    try {
        ...
        addMessage('ai', "Download failed due to security policy. Please save manually.");
    }
}
...
function clearChat() {
    ...
    document.getElementById('chatMessages').innerHTML = `
        <div class="welcome-message">
            ...
            <p>I'm your intelligent assistant, created by KingzAlkhasim. Ask me anything!</p>
        </div>
    `;
    saveData();
}
...
async function sendMessage() {
    ...
    try {
        if (isImageGenerationRequest(text)) {
            ...
            if (apiProvider !== 'openai') throw new Error("Video generation is OpenAI only.");
...
function showProcessing() {
    ...
    procDiv.innerHTML = `<div class="message-avatar"><i class="fas fa-robot"></i></div><div class="message-content"><div class="processing-indicator"><span></span><span class="processing-dots"><span class="processing-dot"></span><span class="processing-dot"></span><span class="processing-dot"></span></span></div></div>`;
...
function isImageGenerationRequest(text) {
    ...
    const keywords = ['generate', 'create', 'make', 'draw', 'show me', 'image of', 'picture of'];
    const imageWords = ['image', 'picture', 'photo', 'drawing', 'art'];
    return keywords.some(kw => lowerCaseText.includes(kw)) && imageWords.some(img => lowerCaseText.includes(img));
}

function isVideoGenerationRequest(text) {
    return text.toLowerCase().includes('video of') || text.toLowerCase().includes('animate');
}
...
async function getAiResponse(text, imageUrl) {
    ...
    if (apiProvider === 'gemini') return await callGeminiAPI(text, imageUrl, apiKey);
    if (apiProvider === 'openai') return await callOpenAI(text, imageUrl, apiKey);
...
async function callGeminiAPI(text, imageUrl, key) {
    ...
    const parts = [];
    const systemInstruction = `You are K-XpertAI. Your creator is Alkhassim Lawal Umar (KingzAlkhasim) of kingxTech Company. Only state this if asked.`;
